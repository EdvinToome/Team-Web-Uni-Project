...
<body>
    ...
    <main class="container2">
        ...
        $query = "select * from bikes where id = $_GET[id]";
        ...
        <div class="right-column">
            <div class="product-description">
                <span>'.$row["name"].'</span>
                <h1 id="product-heading">More sports, less cars!</h1>
                <p>'.$row["description"].'</p>
            </div>
            ...
            <form action="single_bike.php?id='.$row["id"].'" method="post">
             <input class="cart-btn" type="submit" name="rentBike" value="Rent now">
             </form>
            </div>
        </div>';
       // Check user balance on pressing rent button
        if (isset($_POST['rentBike'])) {
            ...
            if ($row["balance"] < $price) {
                ...
                $result = $conn->query($query);
                echo '<script>document.getElementById("product-heading").innerHTML = "You have sucessfully rented the bike!";</script>';
            }
        }
        $conn->close();
        exit();
